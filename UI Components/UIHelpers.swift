...
struct StatusChip: View {
    ...
    var body: some View {
        Text(text.uppercased())
            ...
            .minimumScaleFactor(0.7)           // shrink instead of clipping
            .padding(.horizontal, 8)
            .padding(.vertical, 4)
...
// MARK: - Section headers & info rows

/// Simple stacked title / subtitle header used above cards or groups.
// MARK: - Section headers / info rows / tags
...
/// Glassy background card used under sections.
...
/// A single row for label/value metadata, optional icon on the left.
struct InfoRow: View {
    let label: String
    let value: String
    var icon: String? = nil
...
/// Small pill used for inline tags (e.g. "HIDDEN", "3D MODEL", "In your stack").
struct TagPill: View {
    let text: String
    var icon: String? = nil
    var tint: Color? = nil   // supports TagPill(text: "HIDDEN", tint: .red)

    private var backgroundGradient: LinearGradient {
        let base = tint ?? Color.white.opacity(0.35)
